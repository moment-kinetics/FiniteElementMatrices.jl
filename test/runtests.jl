...
function test_2D_linear_operators(;nodes::node_type=GLL,
            ngrid_x1=5::Int64,ngrid_x2=4::Int64,
            atol=1.0e-13::Float64)
    # x1
    # endpoints of grid
    y1 = 1.2
    y0 = -1.3
    x1 = reference_nodes(nodes,ngrid_x1)
    scale_x1 = 0.5*(y1-y0)
    shift_x1 = 0.5*(y0+y1)
    coordinate_x1 = ElementCoordinates(x1,scale_x1,shift_x1)
    # x2
    # endpoints of grid
    y1 = 0.9
    y0 = -0.78
    x2 = reference_nodes(nodes,ngrid_x2)
    scale_x2 = 0.5*(y1-y0)
    shift_x2 = 0.5*(y0+y1)
    coordinate_x2 = ElementCoordinates(x2,scale_x2,shift_x2)

    M_x1 = zeros(ngrid_x1,ngrid_x1)
    M_x2 = zeros(ngrid_x2,ngrid_x2)
    M_2D = zeros(ngrid_x1,ngrid_x2,ngrid_x1,ngrid_x2)
    M_2D_err = zeros(ngrid_x1,ngrid_x2,ngrid_x1,ngrid_x2)
    M_2D_exact = zeros(ngrid_x1,ngrid_x2,ngrid_x1,ngrid_x2)
    # test that a 2D matrix with polynomial kernel can be
    # calculated with an identical result to the
    # product of 1D matrices with the same kernels
    for power1 in 0:3
        for power2 in 0:3
...
function runtests()
    @testset "FiniteElementMatrices" begin
        ...
        println("test 2D operators:")
        for nodes in nodes_list
            println("    -- test: $(nodes) ngrid_x1=5 ngrid_x2=4 poly")
            test_2D_linear_operators(;nodes=nodes,
                ngrid_x1=5, ngrid_x2=4, atol=1.0e-13)
